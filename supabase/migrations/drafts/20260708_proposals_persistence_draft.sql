...
--
-- Diseño base:   docs/product/proposals-persistence-design-15j0.md (FASE 15J-0)
-- Revisión SQL:  docs/product/proposals-sql-draft-review-15j.md    (FASE 15J)
--
-- ADITIVO: crea la tabla `proposals`. NO modifica de forma destructiva ninguna
-- tabla existente. NO migra datos. NO conecta con `opportunities` (queda para 15O).
--
...
--   * CHECK constraints sobre TEXT (NO enums Postgres). Motivo: los CHECK se
--     evolucionan con ALTER CONSTRAINT (sin ALTER TYPE ni locks pesados). Es el
--     patrón ya establecido en leads y en el Motor IA. (Decisión confirmada en
--     FASE 15J: se descartaron los tipos nativos proposal_status/proposal_source.)
--   * set_updated_at() — función global ya usada por leads y el Motor IA.
--   * FKs de responsables → profiles(id) (NO auth.users). En este proyecto
--     profiles.id == auth.uid(); referenciar profiles mantiene la integridad con
--     el resto del schema y con los helpers de rol (is_in_my_team recibe un
--     profiles.id).
--   * RLS con helpers existentes en la base remota (STABLE SECURITY DEFINER):
--       is_admin_or_direccion()
--       get_user_role()
--       is_in_my_team(uuid)
--     Estas funciones fueron creadas directamente en Supabase (no hay CREATE
--     FUNCTION local). La precondición de abajo aborta si no existen.
--
-- SIN OpenAI · SIN proveedor de IA · SIN Compliance / legal_review (ninguna forma).
--
-- NO EJECUTAR en remoto ni en dev hasta autorización explícita.
-- Ver checklist previo en docs/product/proposals-sql-draft-review-15j.md.
-- =============================================================================
...
--   * Campos denormalizados (summary/target_audience/problem/opportunity/
--     proposed_offer) existen para listados y búsqueda.
...
--
-- FIN DEL DRAFT — NO APLICAR SIN LEER docs/product/proposals-sql-draft-review-15j.md
-- =============================================================================
